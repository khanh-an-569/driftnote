...
4. Preserve headings, document TOC hierarchy, lists, tables, code fences, timestamps, source links, remote images, math source, Quarto title blocks, and expandable `<details>` when available. If these structures matter, save the authorized rendered DOM/HTML as UTF-8 and pass it raw with `--content-file` or `--html-file`; do not reduce it to `innerText`. / Giữ tiêu đề, cây TOC, danh sách, bảng, code fence, timestamp, liên kết, ảnh, công thức, title block Quarto và `<details>`; nếu chúng quan trọng, dùng DOM/HTML UTF-8 với `--html-file`, không dùng text phẳng.
...
For a full-page Chrome capture, pass authorized raw DOM/HTML through `--content-file` or `--html-file`. The helper detects HTML structure from the data, converts it, and rejects a purported HTML file that contains only text. Browser text from `getTabContext` or `innerText` has no hyperlink destinations. The helper rejects a Chrome text-only capture unless `--allow-text-only` explicitly accepts a text-only note. Use that flag only when missing links are acceptable, and describe the result honestly. The helper cannot infer lost HTML semantics from flattened text. Use `--refresh-existing` only for an exact duplicate and only after the user requests repair; the personal-notes section remains intact.
