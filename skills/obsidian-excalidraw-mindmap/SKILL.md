...
3. Write the outline to a JSON file and run `scripts/generate_excalidraw.py`. The script never reads note content itself and makes no content judgments — it only lays out, styles, and publishes exactly the outline it is given. Pass `--config-file` pointing at the vault's `driftnote.yaml` only when the user wants diagrams placed under its `excalidraw_output_dir` instead of next to the source note; otherwise omit it.
